...
// (either as a copy or as a submodule), and only build the backends you need. If you are
// new to Dear ImGui, read documentation from the docs/ folder + read the top of
// imgui.cpp. Read online: https://github.com/ocornut/imgui/tree/master/docs

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2022-11-09: OpenGL: Reverted use of glBufferSubData(), too many corruptions issues +
//  old issues seemingly can't be reproed with Intel drivers nowadays (revert 2021-12-15
//  and 2022-05-23 changes). 2022-10-11: Using 'nullptr' instead of 'NULL' as per our
//  switch to C++11. 2022-09-27: OpenGL: Added ability to '#define
//  IMGUI_IMPL_OPENGL_DEBUG'. 2022-05-23: OpenGL: Reworking 2021-12-15 "Using buffer
//  orphaning" so it only happens on Intel GPU, seems to cause problems otherwise. (#4468,
//  #4825, #4832, #5127). 2022-05-13: OpenGL: Fix state corruption on OpenGL ES 2.0 due to
//  not preserving GL_ELEMENT_ARRAY_BUFFER_BINDING and vertex attribute states.
//  2021-12-15: OpenGL: Using buffer orphaning + glBufferSubData(), seems to fix leaks
//  with multi-viewports with some Intel HD drivers. 2021-08-23: OpenGL: Fixed ES 3.0
...
//  (imgui_impl_opengl3_loader.h), removing requirement and support for third-party
//  loader. 2021-06-29: Reorganized backend to pull data from a single structure to
//  facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX).
//  2021-06-25: OpenGL: Use OES_vertex_array extension on Emscripten + backup/restore
//  current state. 2021-06-21: OpenGL: Destroy individual vertex/fragment shader objects
//  right after they are linked into the main shader. 2021-05-24: OpenGL: Access
//  GL_CLIP_ORIGIN when "GL_ARB_clip_control" extension is detected, inside of just
//  OpenGL 4.5 version. 2021-05-19: OpenGL: Replaced direct access to ImDrawCmd::TextureId
//  with a call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-04-06: OpenGL:
//  Don't try to read GL_CLIP_ORIGIN unless we're OpenGL 4.5 or greater. 2021-02-18:
//  OpenGL: Change blending equation to preserve alpha in output buffer. 2021-01-03:
//  OpenGL: Backup, setup and restore GL_STENCIL_TEST state. 2020-10-23: OpenGL: Backup,
//  setup and restore GL_PRIMITIVE_RESTART state. 2020-10-15: OpenGL: Use
//  glGetString(GL_VERSION) instead of glGetIntegerv(GL_MAJOR_VERSION, ...) when the later
//  returns zero (e.g. Desktop GL 2.x) 2020-09-17: OpenGL: Fix to avoid compiling/calling
//  glBindSampler() on ES or pre 3.3 context which have the defines set by a loader.
...
//  render loop. 2019-03-15: OpenGL: Added a GL call + comments in
...
//  optional so 3.2 context won't fail if the function is a nullptr pointer. 2018-03-06:
...
// function pointers. Helper libraries are often used for this purpose! Here we are using
// our own minimal custom loader based on gl3w. In the rest of your app/engine, you can
// use another loader of your choice (gl3w, glew, glad, glbinding, glext, glLoadGen,
// etc.). If you happen to be developing a new feature for this backend
...
// https://github.com/dearimgui/gl3w_stripped/releases Changes to this backend using new
// APIs should be accompanied by a regenerated stripped loader version.
//#define IMGL3W_IMPL
...
struct ImGui_ImplOpenGL3_Data {
    GLuint GlVersion; // Extracted at runtime using GL_MAJOR_VERSION, GL_MINOR_VERSION
                      // queries (e.g. 320 for GL 3.2)
    char GlslVersionString[32]; // Specified by user or detected based on compile time GL
                                // settings.
    GLuint FontTexture;
    GLuint ShaderHandle;
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    */
    ...
    // IF YOU GET A CRASH HERE: it probably means the OpenGL function loader didn't do its
    // job. Let us know!
...
// up/restoring every OpenGL state explicitly. This is in order to be able to run within
// an OpenGL engine that doesn't do so.
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        //   orphaning+glBufferSubData() following reports of leaks on Intel GPU when
        //   using multi-viewports on Windows.
        // - After this we kept hearing of various display corruptions issues. We started
        // disabling on non-Intel GPU, but issues still got reported on Intel.
        // - We are now back to using exclusively glBufferData(). So bd->UseBufferSubData
        // IS ALWAYS FALSE in this code.
